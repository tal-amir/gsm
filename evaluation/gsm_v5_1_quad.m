...
%       floating point arithmetic. To use this feature, <use_mex> must be set to false.
...
function [mu, theta, info] = alg8(numcast, z, k, gamma, use_mex, full_sort, slow_mode)
d = numcast(numel(z));
...
elseif gamma < 0
    [mu, theta, info] = alg8(numcast, -z, k, -gamma, use_mex, full_sort, slow_mode);
    mu = -mu;
elseif gamma == inf
    a = min(maxk(z,k));
    I_eq = find(z == a);
...
elseif 2*k > d
    [mu, theta, info] = alg8(numcast, -z, d-k, gamma, use_mex, full_sort, slow_mode);
    mu = mu + sum(z);
    theta = 1-theta;
else
    % Here 1 <= k <= d/2 and 0 < gamma < inf
    [mu, theta, info] = calc_gsm_main(numcast, z, k, gamma, use_mex, full_sort, slow_mode);
end
end


% This function calculates the GSM under the assumption 1 <= k <= d/2, 0 < gamma < inf.
function [mu, theta, info] = calc_gsm_main(numcast, z, k, gamma, use_mex, full_sort, slow_mode)
d = numel(z);
dl = max(1,2*k-2);
dr = d-dl;
...
if use_mex
    if slow_mode
        warning('<slow_mode> = true is only applicable with <use_mex> = false');
    end
    
    [mu, theta, dl, bz, bzl, bzr, delta, alpha, log_alpha] = gsm_v5_1_mex_quad(z, k, gamma);
else
    error('This only works with use_mex');
...
for r=d:-1:1
    ...
    for q=1:nmin(s,d-r)
        ...
        if eta <= 0
            ...
            % I think this case is possible only if z is not fully
            % sorted in decreasing order (unverified)
